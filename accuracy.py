...
with open('sbert_simscores.data','rb') as filehandle:
    sbert_simscores= pickle.load(filehandle)

with open('use_simscores.data','rb') as filehandle:
    use_simscores= pickle.load(filehandle)

# combining the two similarity vectors for each query- final_simscores is also a (440* 18k) 2D numpy array.
final_simscores= np.add(use_simscores*(6/7), sbert_simscores*(1/7))
...
print('The accuracy of our final model is:', str(accuracy)+'%')